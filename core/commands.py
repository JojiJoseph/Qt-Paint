...
class LineCommand:
    def __init__(self, buffer, x1, y1, x2, y2) -> None:
        self.buffer = buffer
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
    ...
    def execute(self):
        ...
        cv2.line(self.buffer, (self.x1, self.y1), (self.x2, self.y2), (0, 255, 0), 1)
    def undo(self):
        self.buffer[self.top:self.bottom+1, self.left:self.right+1] = self.undo_mask.copy()


class FreehandCommand:
    def __init__(self, buffer, points) -> None:
        self.buffer = buffer
        self.points = points
...
class FloodFillCommand:
    def __init__(self, buffer, seed, rect) -> None:
        self.buffer = buffer
        self.seed = seed
        self.rect = rect
        print(self.rect)
    ...
    def execute(self):
        ...
        cv2.floodFill(self.buffer, None, self.seed, (0, 255, 0))
